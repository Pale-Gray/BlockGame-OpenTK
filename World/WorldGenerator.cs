    ...
    internal class WorldGenerator
    {
        ...
        public static void Generate(World world, Vector3i cameraChunkPosition)
        {
            ...
            if (CurrentRadius <= MaxRadius)
            {
                ...
                /*
                for (int i = 0; i < ChunksToGenerate.Length; i++)
                {

                    if (!world.WorldChunks.ContainsKey(ChunksToGenerate[i]))
                    {

                        world.WorldChunks.Add(ChunksToGenerate[i], new Chunk(ChunksToGenerate[i]));
                        PassOneQueue.Enqueue(ChunksToGenerate[i]);

                    }
                    else
                    {

                        if (world.WorldChunks[ChunksToGenerate[i]].MeshState == MeshState.Meshed)
                        {

                            amountMeshed++;

                            if (world.WorldChunks[ChunksToGenerate[i]].ChunkState == ChunkState.NotReady)
                            {

                                ChunkBuilder.CallOpenGL(world.WorldChunks[ChunksToGenerate[i]]);

                            }

                        }

                        if (world.WorldChunks[ChunksToGenerate[i]].GenerationState == GenerationState.Generated && world.WorldChunks[ChunksToGenerate[i]].MeshState == MeshState.NotMeshed)
                        {

                            if (AreNeighborsCertainGenerationState(ChunksToGenerate[i], GenerationState.Generated, world))
                            {

                                Console.WriteLine("yes");
                                if (world.WorldChunks[ChunksToGenerate[i]].QueueMode == QueueMode.NotQueued)
                                {

                                    world.WorldChunks[ChunksToGenerate[i]].QueueMode = QueueMode.Queued;
                                    MeshPassQueue.Enqueue(ChunksToGenerate[i]);
                        ...
                        if (world.WorldChunks[ChunksToGenerate[i]].GenerationState == GenerationState.PassOne)
                        {

                            if (AreNeighborsCertainGenerationState(ChunksToGenerate[i], GenerationState.PassOne, world))
                            {
                                ...
                                if (world.WorldChunks[ChunksToGenerate[i]].QueueMode == QueueMode.NotQueued)
                                {
                                    ...
                                    world.WorldChunks[ChunksToGenerate[i]].QueueMode = QueueMode.Queued;
                                    PassTwoQueue.Enqueue(ChunksToGenerate[i]);
                ...
                if (amountMeshed >= ChunksMeshedLength)
                {
