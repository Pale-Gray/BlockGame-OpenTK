...
vec3 calculateSunVector(int tickTime)
{
    ...
    return vec3(cos(radians((tickTime / 108000.0) * 360.0)), sin(radians((tickTime / 108000.0) * 360.0)), 0);
...
void main()
{
    ...
    vec4 color = texture(uSkyColor, vec2(mod((uTickTime / 108000.0), 1.0), intensity));
    float val = 1.0;
    if (chebDist(rayDirection, calculateSunVector(uTickTime)) < 0.1) val = 0.0;
    color *= val;
